...
_kube_ps1_init() {
  ...
  case "${KUBE_PS1_SHELL}" in
    "zsh")
      # TODO: consider global name prefix
      OPEN_ESC="%{"
      CLOSE_ESC="%}"
      DEFAULT_BG="%k"
      DEFAULT_FG="%f"
...
_kube_ps1_color_fg() {
  ...
  case "${1}" in
    ...
    # 256
    [0-2][1-9][0-6]) KUBE_PS1_FG_CODE="${1}";;
    *) KUBE_PS1_FG_CODE=9;;
  esac
  ...
  if [[ "${KUBE_PS1_SHELL}" == "zsh" ]]; then
    ...
    if tput setaf 1 &> /dev/null; then
      ...
      KUBE_PS1_FG_CODE="${DEFAULT_FG}"
    fi
  fi
  echo ${OPEN_ESC}${KUBE_PS1_FG_CODE}${CLOSE_ESC}
}
...
_kube_ps1_color_bg() {
  ...
  case "${1}" in
    ...
    # 256
    [0-2][1-9][0-6]) KUBE_PS1_BG_CODE="${1}";;
...
kube_ps1() {
  ...
  local KUBE_PS1
  local KUBE_PS1_RESET_COLOR="${OPEN_ESC}${DEFAULT_FG}${CLOSE_ESC}"
  ...
  [[ -n "${KUBE_PS1_BG_COLOR}" ]] && KUBE_PS1+="${OPEN_ESC}${DEFAULT_BG}${CLOSE_ESC}"
